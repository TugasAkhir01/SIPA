...
const Approval = () => {
    ...
    const handleDelete = async () => {
        ...
        try {
            const res = await fetch(`http://sippak-be.up.railway.app/api/violations/${deleteId}`, {
                method: 'DELETE',
    ...
    const updateStatusApproval = async (id, status) => {
        try {
            const res = await fetch(`http://sippak-be.up.railway.app/api/violations/${id}/status`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: `Bearer ${token}`
        ...
        <Box display="flex" flexDirection="column" sx={{ height: '100vh' }}>
            ...
            >
                ...
                <DialogContent dividers sx={{ px: 4, py: 3 }}>
                    ...
                    >
                        <Avatar variant="square" sx={{ width: 90, height: 90, mb: 2 }} src={form.foto ? `http://sippak-be.up.railway.app/uploads/data_pelanggaran/photo/${form.foto}` : undefined} />
                        <Button
                            variant="contained"
                            component="label"
                            size="small"
            ...
            >
                ...
                <DialogActions sx={{ justifyContent: 'center', mb: 2 }}>
                    ...
                    <Button
                        onClick={async () => {
                            try {
                                const res = await fetch(`http://sippak-be.up.railway.app/api/violations/${approvalDialog.id}/status`, {
                                    method: 'PUT',
                                    headers: {
                                        'Content-Type': 'application/json',
                                        Authorization: `Bearer ${token}`
